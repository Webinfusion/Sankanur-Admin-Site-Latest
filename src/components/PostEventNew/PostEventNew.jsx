...
import {
  insertPostNewEventConst,
  updatePostEventConst,
...
function PostEventNew({ propsData }) {
  ...
  const [popUpInstance, setPopUpInstance] = useState({
    open: false,
    status: "",
    message: "",
  });
  ...
  const validation = (formData) => {
    ...
    if(propsData) {
      var dataKeys = ['title', 'date', 'description', 'content', 'time', 'images']
    } else {
      var dataKeys = Object.keys(formData).filter(
        (key) => !key.includes("speaker")
      );
    }

    dataKeys.some((fieldName) => {
      if (fieldName === "images" && formData[fieldName].length !== 3) {
        triggerPopup("error", `All 3 Images are Mandatory !!`);
        status = false;
        return true;
      }
      ...
      if (!formData[fieldName] || formData[fieldName].length === 0) {
        triggerPopup(
          "error",
          `${fieldName[0].toUpperCase() + fieldName.slice(1)} is Mandatory !!`
        );
        status = false;
        return true;
      }
  ...
  const triggerPopup = (status, message) => {
    ...
    setTimeout(() => {
      setPopUpInstance((prev) => ({ ...prev, open: false }));
    }, 3000);
  };
  ...
  return (
    <ThemeProvider theme={theme}>
      {isLoading && <Loader />}
      {popUpInstance.open && (
        <PopUp status={popUpInstance.status} message={popUpInstance.message} />
      )}

      <Box display={"flex"} justifyContent="space-between" mb={5}>
        <Typography variant="h4" fontWeight={"bold"}>
          Post Event {(initialData && propsData) ? 'Update': 'New'}
        </Typography>
        <Button
          variant="contained"
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          ...
          <ReactQuill
            ...
            className="custom-quill-editor" // Apply a custom class
          />
        </Grid>
        ...
        <Grid item xs={12} md={4}>
          <TextField
            ...
            onChange={(e) => updateFormData("speaker1", e.target.value)}
          />
        </Grid>
        <Grid item xs={12} md={4}>
          <TextField
            ...
            onChange={(e) => updateFormData("speaker2", e.target.value)}
          />
        </Grid>
        <Grid item xs={12} md={4}>
          <TextField
            fullWidth
            label="Speaker 3"
            value={formData.speaker3}
            onChange={(e) => updateFormData("speaker3", e.target.value)}
          />
        </Grid>
        <Grid item xs={12} md={4}>
          <TextField
            fullWidth
            label="Speaker 4"
            value={formData.speaker4}
            onChange={(e) => updateFormData("speaker4", e.target.value)}
          />
        </Grid>
        <Grid item xs={12} md={4}>
          <TextField
