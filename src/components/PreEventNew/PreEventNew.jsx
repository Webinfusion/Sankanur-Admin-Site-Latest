...
function PreEventNew({ propsData }) {
  ...
  const validation = (formData) => {
    ...
    dataKeys.some((fieldName) => {
      if (fieldName === "images" && formData[fieldName].length !== 1) {
        triggerPopup("error", `Image is Mandatory !!`);
        status = false;
        return true;
      }
    ...
    <ThemeProvider theme={theme}>
      ...
      <Box display={"flex"} justifyContent="space-between" mb={5}>
        <Typography variant="h4" fontWeight={"bold"}>
          Pre Event Table {(initialData && propsData) ? 'Update': 'New'}
        </Typography>
        <Button
          variant="contained"
